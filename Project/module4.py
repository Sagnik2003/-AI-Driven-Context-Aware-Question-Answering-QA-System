...
All-in-one pipeline combining:
 - Bayesian Intent Reasoning (Module A)
 - Search-based Retrieval (Module B) with intent-aware BFS and A* (TF-IDF heuristic)
 - Planner (Module C) that builds a plan and returns structured final_plan
 - RL Policy (Module D) with Q-learning on discrete state space, used to refine planner decisions

Usage:
  - Import functions:
      from pipeline import plan_for_query, get_default_rl_policy
      result = plan_for_query(query)   # silent by default
  - Run demo:
      python pipeline.py
    This runs a small demo over example queries and writes demo_plans.json.

Note: training Q-table is done lazily inside RLPolicy if no Q is provided; you can call train_q_table()
and save the produced Q-table to disk for reuse.
"""
